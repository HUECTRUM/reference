...
void naive(int n) {
    ...
    for (auto &x: pr) {
        ...
        if (nm > n) break;
        int cp3 = 0;
        for (auto &y: pr) if (x != y) {
                if (x * x * x * y > n) break;
                ++cp3;
            }
        p3 += cp3;
        //cout << "added " << cp3 << " for prime " << x << endl;
    }

    int pqr = 0;
    for (int i = 0; i < pr.size(); ++i) {
        int pr1 = pr[i];
        if (pr1 * pr1 * pr1 > n) break;
        for (int j = i + 1; j < pr.size(); ++j) {
            int pr2 = pr[j];
            if (pr2 * pr2 * pr1 > n) break;
            for (int k = j + 1; k < pr.size(); ++k) {
                int pr3 = pr[k];
                if (pr1 * pr2 * pr3 > n) break;
                ++pqr;
            }
        }
    }

    cout << p7 << " " << p3 << " " << pqr << " " << p7 + p3 + pqr << endl;
}
...
signed main() {
    ...
    int p3 = 0;
    for (auto &x: pr) {
        int nm = x * x * x, dv = n / nm;
        if (nm > n || dv < 2) break;
        int curr = p.primeCnt[dv] - (dv >= x ? 1 : 0);
        //cout << "added " << curr << " for prime " << x << endl;
        p3 += curr;
    }
    cout << "e3" << endl;
    ...
    for (int i = 0; i < pr.size(); ++i) {
        ...
        for (int j = i + 1; j < pr.size(); ++j) {
            ...
            int rem = n / (pr1 * pr2);
            int div1 = p.primeCnt[rem], div2 = j + 1;
            int add = div1 - div2;
