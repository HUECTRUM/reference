...
int phi[MAXN], mu[MAXN], gcs[MAXN], cnt[MAXN], lp[MAXN], muSum[MAXN];
Mint phiSum[MAXN], gcsSum[MAXN];

void sieve(int n = MAXN) {
    fill(is_composite, is_composite + n, false);
    phi[1] = 1; mu[1] = 1; gcs[1] = 1; lp[1] = 1;
    for (int i = 2; i < n; ++i) {
        if (!is_composite[i]) {
            primes.push_back(i), cnt[i] = 1;
            lp[i] = i;
            //f(p)
            phi[i] = i - 1; mu[i] = -1; gcs[i] = 2 * i - 1;
        ...
        for (int j = 0; j < primes.size () && i * primes[j] < n; ++j) {
            ...
            if (i % primes[j] == 0) { //p[j] divides i
                ...
                if (div == 1) {
                    ...
                    mu[i * primes[j]] = 0;
                    gcs[i * primes[j]] = (pk * ((k + 1) * primes[j] - k)) / (primes[j]);
                } else {
                    ...
                    gcs[i * primes[j]] = gcs[i / primePowers[j][cnt[i]]] * gcs[primePowers[j][cnt[i]] * primes[j]];
                }
                ...
                gcs[i * primes[j]] = gcs[i] * gcs[primes[j]];
...
Mint getGcsSum(int n, int x) { return x < MAXN ? gcsSum[x] : divGcs[n / x]; }
...
void runMuSum(int n, int last) {
    reprv(i, last) {
        ...
        reps(f, 2, sqr + 1) val -= getMuSum(n, x / f);
        reps(f, 1, sqr + 1) val -= mu[f] * x / f;
...
void runPhiSum(int n, int last) {
    reprv(i, last) {
        ...
        reps(f, 1, sqr + 1) val += getMuSum(n, x / f) * f;
        reps(f, 1, sqr + 1) val += mu[f] * sm(x / f);

        divPhi[i] = val;
    }
}

void runGcsSum(int n, int last) {
    reprv(i, last) {
        if (!i) break;
        int x = n / i, sqr = sqroot(x);

        Mint val = - getPhiSum(n, sqr) * sm(sqr);
        reps(f, 1, sqr + 1) val += getPhiSum(n, x / f) * f;
        reps(f, 1, sqr + 1) val += phi[f] * sm(x / f);
...
signed main() {
    ...
    if (n < MAXN) cout << phiSum[n];
        ...
        divMu.resize(last); divPhi.resize(last); divGcs.resize(last);
        runMuSum(n, last); runPhiSum(n, last); runGcsSum(n, last);
