...
int lyn(vi &w, int n) {
    int ans = 1;
    reps(i, 2, n + 1) {
        if (w[i] > w[i - ans]) ans = i;
        else if (w[i] < w[i - ans]) return ans;
    }
    return ans;
}

bool isNeck(vi &w, int n) {
    int ans = 1;
    reps(i, 2, n + 1) {
        if (w[i] > w[i - ans]) ans = i;
        else if (w[i] < w[i - ans]) return false;
    }
    return !(n % ans);
}

void lNeck(vi &w, vi &ans, int n, int a_size) {
    reps(i, 1, n + 1) ans[i] = w[i];

    while (!isNeck(ans, n)) {
        int ln = lyn(ans, n);
        ans[ln]--;
        reps(i, ln + 1, n + 1) ans[i] = a_size;
    }
}

vvi buildDp_B(vi &v, int n, int alph) {
    vvi dp(n + 1, vi(n + 1)); dp[0][0]=1;

    reps(i, 1, n + 1) {
        dp[i][i] = 0;
        reprv(j, i) dp[i][j]= dp[i][j + 1] + (alph - v[j + 1]) * dp[i - j - 1][0];
    }
    return dp;
}

vvi buildDp_suff(vi &v, int n) {
    vvi dp(n + 2, vi(n + 2));

    reps(i, 2, n + 1) {
        int s = i;
        reps(j, i, n + 1) {
            if (v[j] > v[j - s + 1]) s = j + 1;
            dp[i][j] = j - s + 1;
        }
    }
    return dp;
...
int T_card(vi &v, int n, int alph) {
    vvi b = buildDp_B(v, n, alph), suf = buildDp_suff(v, n);
    int ans = lyn(v, n);

    reps(t, 1, n + 1) {
        rep(j, n) {
            if (j + t <= n) ans += b[t - 1][0] * (v[j + 1] - 1) * binpow(alph,n - t - j);
            else {
                int s = (j < n - t + 2) ? 0 : suf[n - t + 2][j];
                if (v[j + 1] > v[s + 1]) ans += b[n - j + s][s + 1] + (v[j + 1] - v[s + 1]-1) * b[n - j - 1][0];
            }
        }
    }
    return ans;
...
int occ_pos(vi &w, int n, int alph) {
    //case 1: wrap
    int t = 0; while (w[t + 1] == alph && t + 1 <= n) t++;
    int j = t; while (w[j + 1] == 1 && j + 1 <= n) j++;
    if (t >= 1 && j == n) return pows[n] - t + 1;

    //case 2: neck
    if (isNeck(w, n)) return T_card(w, n, alph) - lyn(w, n) + 1;
    ...
    while (!isNeck(neck, n)) {
        ++s;
        reps(i, 1, n + 1) neck[i] = (i + s <= n)? w[i + s] : w[i + s - n];
    }
    int ln = lyn(neck, n);
    if (s != t) return occ_pos(neck, n, alph) + ln - s;
    if (ln < n) return occ_pos(neck, n, alph) - s;

    vi prev(n + 1);
    reps(i, n - s + 1, n + 1) neck[i] = 1;
    lNeck(neck,prev, n, alph);

    return occ_pos(prev, n, alph) + lyn(prev, n) - s;
}

void precompPows(int n, int alph) {
    pows = vi(n + 1); pows[0] = 1;
    reps(i, 1, n + 1) pows[i] = pows[i - 1] * alph;
}

const int modSeq = 1e12, LEN = 12, ALPH = 10, MOD = 1234567891;

signed main() {
    int n = 1e7;

    precompPows(LEN, ALPH);
    auto genNext = [&](int x) { return ((__uint128_t)x * 920461u + 800217387569u) % modSeq; };
    ...
    auto fillDigs = [&](int x) {
        reprv(k, LEN) v0[k] = x % 10, x /= 10;
        rep(i, LEN) v1[i + 1] = v0[i] + 1;
    };
    rep(i, n) {
        seq = genNext(seq); fillDigs(seq);
        pairs.emplace_back(occ_pos(v1, LEN, ALPH), seq);
    }

    std::stable_sort(pairs.begin(), pairs.end(),[](auto &x, auto &y){ return x.first < y.first; });

    int ans = 0;
    rep (i, pairs.size()) ans = (ans + ((__int128)(i + 1) * pairs[i].second)) % MOD;
    cout << ans;
}
