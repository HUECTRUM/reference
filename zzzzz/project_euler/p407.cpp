...
#define Mint modint1e9
#define vmint vector<modint1e9>
#define vvmint vector<vector<modint1e9>>



pair<int, int> ext_gcd(int a, int b) {
    if (a == 0) return {0, 1};
    pair<int, int> rec = ext_gcd(b % a, a);
    return { rec.second - (b / a) * rec.first, rec.first };
}

int mod_inv(int a, int m) {
    int cand = ext_gcd(a, m).first;
    return ((cand % m) + m) % m;
}
...
int getIdemp(int x) {
    ...
    rep(mask, (1ll << n)) {
        vct<Congruence> cong;
        rep(i, n) {
            cong.emplace_back((mask >> i) & 1, fct[i]);
        }

        int curIdemp = crt(cong);
        chmax(best, curIdemp);
    }
    return best;
}
