...
int indGeq(vi &v, int x) { return std::lower_bound(v.begin(), v.end(), x) - v.begin(); }
...
int getVectorCompressed(int val, vi &v) { return lower_bound(v.begin(), v.end(), val) - v.begin(); }
...
const int MAXV = 1010;
vector<int> lp(MAXV + 1, 0), pr;

void linsieve() {
    lp[1] = 1;
    for (int i = 2; i <= MAXV; ++i) {
        if (!lp[i]) lp[i] = i, pr.push_back(i);
        for (int j = 0; i * pr[j] <= MAXV; ++j) {
            lp[i * pr[j]] = pr[j];
            if (pr[j] == lp[i]) break;
        }
    }
...
bitset<MAXN> factorize(int n) {
    bitset<MAXN> ans;
    for (auto &i: pr) {
        if (n % i) continue;
        int cnt = 0;
        while (n % i == 0) n /= i, ++cnt;
        if (cnt & 1) ans.set(getVectorCompressed(i, pr));
    }
    if (n > 1) ans.set(getVectorCompressed(n, pr));
    return ans;
}

vector<bitset<MAXN>> basisRep(MAXN);
vector<pair<int, bitset<MAXN>>> otherReps;
struct XorBasis {
    vector<dta> basis;
    int sz = 0;
    ...
    void addToBasis(int i, dta &mask) {
        basis[i] = mask; ++sz;
    }
    ...
    bool add(dta f) {
        ...
        for (int i = MAXN - 1; i >= 0; --i) {
            ...
            if (!basis[i].val) {
                addToBasis(i, f);
                rep[i] = 1; basisRep[i] = rep;
                return true;
            }
            f.msk ^= basis[i].msk; rep ^= basisRep[i];
        }
        otherReps.emplace_back(f.idx, rep);
        return false;
    }

    bool has(bitset<MAXN> f) {
        for (int i = MAXN - 1; i >= 0; --i) {
            if (!f[i]) continue;
            if (!basis[i].val) return false;
            f ^= basis[i].msk;
        }
        return true;
    }
...
signed main() {
    ...
    rep(i, n) {
        cin >> v[i].val;
        v[i].sgn = (v[i].val < 0 ? -1 : 1); v[i].val = abs(v[i].val);
        v[i].msk = factorize(v[i].val);
    }

    //greedy max
    std::sort(v.begin(), v.end(), [&](dta &d1, dta &d2) {
    ...
    if (!sgn) {
        Mint ans = 1;
        rep(i, MAXN) if (xb.basis[i].val) ans *= xb.basis[i].val;
        cout << ans;
        return 0;
    }

    ld bestChange = 0; int bestIdx = -1, bestRep = -1;
    rep(i, MAXN) if (xb.basis[i].val) {
            int curSgn = xb.basis[i].sgn;
            for (auto &[nbIdx, nbRep]: otherReps) if (nbRep[i] && v[nbIdx].sgn != curSgn) {
                    ld diff = (ld)v[nbIdx].val/xb.basis[i].val;
                    if (diff <= bestChange) continue;
                    bestChange = diff;
                    bestIdx = i;
                    bestRep = nbIdx;
                }
    ...
    if (bestIdx != -1) { //swap exists
        Mint ans = v[bestRep].val;
        rep(i, MAXN) if (xb.basis[i].val && i != bestIdx) ans *= xb.basis[i].val;
        cout << ans;
        return 0;
    }

    //swap doesnt exist - build negative
    std::sort(v.begin(), v.end(), [&](dta &d1, dta &d2) {
        return d1.val < d2.val;
    });
    XorBasis xb1;
    Mint ans = 1;
    rep(i, n) {
        bool added = xb1.add(v[i]);
        if (added) ans *= (v[i].val * v[i].sgn);
    }
    cout << ans;
}