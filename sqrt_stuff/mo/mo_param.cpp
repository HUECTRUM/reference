...
template<typename R> struct MoOps {
    ...
    virtual R getResult(int param) = 0;
};

const int MAXV = 251000,  BLSZ = 500, MAXBLS = MAXV / BLSZ;
struct ShuffleOps: MoOps<Mint> {
    vi v, blockFreq, indFreq;
    vector<Mint> blockFact;


    ShuffleOps(vector<int> const &v) : blockFreq(MAXBLS), blockFact(MAXBLS, 1), indFreq(MAXV), v(v) { }

    void add(int idx) {
        int el = v[idx], blck = el / BLSZ, prevFreq = indFreq[el];

        blockFact[blck] *= bm.fact[prevFreq + 1] * bm.invFact[prevFreq];
        blockFreq[blck]++, indFreq[el]++;
    }
    ...
    Mint getResult(int param) {
        int fullFreq = 0; Mint invFacts = 1;
        int curBlck = param / BLSZ, fullBls = curBlck - 1;

        rep(i, fullBls + 1) {
            fullFreq += blockFreq[i];
            invFacts *= blockFact[i];
        }

        int lastBlStart = BLSZ * curBlck;
        reps(i, lastBlStart, param) if (indFreq[i]) {
                fullFreq += indFreq[i];
                invFacts *= bm.fact[indFreq[i]];
            }

        Mint nom = bm.fact[fullFreq], ans = nom / invFacts;
        return ans;
    }
};

struct query { int l, r, param, id; };
template<typename R> struct MO {
    ...
    MO(vector<query> const &qu, MoOps<R> &moOps, int BLOCKSZ = 500) : ops(moOps) {
    ...
    void run() {
        ...
        for (int i = 0; i < q; ++i) {
            ...
            ans[qu.id] = ops.getResult(qu.param);
        }
    }
...
signed main() {
    ...
    ShuffleOps uOps(v);
    MO<Mint> mo = MO<Mint>(qus, uOps);
    mo.run();
    vector<Mint> ans = mo.getResults();
