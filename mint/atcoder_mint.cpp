...
void gen(int x, int y, int xT, int yT, int n, bool include = true, int num = 0) {
    if (x == xT && y == yT) {
        num += v[x][y];
        if (n == 1) v1.pb(num);
        else v2.pb(num);
    ...
    if (x + 1 <= xT) gen(x + 1, y, xT, yT, n, true, num + (include ? v[x][y] : 0));
    if (y + 1 <= yT) gen(x, y + 1, xT, yT, n, true, num + (include ? v[x][y] : 0));
}
...
signed main() {
    ...
    int n, m; cin >> n >> m;
    vvi x(n, vi(n)); v = vvi(n, vi(n));
    rep(i, n) rep(j, n) cin >> x[i][j];

    vi pows(2 * n + 1); pows[0] = 1;
    reps(i, 1, 2 * n + 1) pows[i] = (pows[i - 1] * 10) % m;

    rep(i, n) rep(j, n) {
            int pw = n - 1 - i + n - 1 - j;
            v[i][j] = (x[i][j] * pows[pw]) % m;
        }
    ...
    rep(i, n) {
        int j = n - 1 - i;
        v1.clear(); v2.clear();
        gen(0, 0, i, j, 1); gen(i, j, n - 1, n - 1, 2, false);
        std::sort(v1.begin(), v1.end()); std::sort(v2.begin(), v2.end());

        repe(vl, v1) {
            int search = m - vl;
            int idx = indGeq(v2, search);
            if (idx < v2.size()) {
                int vl2 = v2[idx], sm = (vl + vl2) % m;
                chmax(ans, sm);
            }
            if (idx - 1 >= 0) {
                int vl2 = v2[idx - 1], sm = (vl + vl2) % m;
                chmax(ans, sm);
            }
        }
    }
