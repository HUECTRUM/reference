...
#define vvmint vector<vector<modint1000000007>>
...
signed main() {
    ...
    while (tests--) {
        ...
        SparseTable<int, mx> sptb(v);
        vi cnt(n + 1);

        function<void(int,int,int)> run = [&](int l, int r, int last) {
            if (last < 0 || l > r) return;

            auto [row, idx] = sptb.get(l, r);
            int curLen = r - l + 1, curRows = last - row;
            cnt[curLen] += curRows;
            run(l, idx - 1, row);
            run(idx + 1, r, row);
        };
        ...
        int segs = 0;
        reprv(i, n + 1) if (cnt[i]) {
                if (cnt[i] * i <= need) {
                    segs += cnt[i];
                    need -= cnt[i] * i;
                } else {
                    int used = ceil(need, i);
                    segs += used;
                    need = 0;
                    break;
                }
            }
        cout << need0 - segs << endl;
    }
}
