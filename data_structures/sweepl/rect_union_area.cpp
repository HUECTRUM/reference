...
const int MAXN = 2097152, INF = 1e18;
...
struct RectArea {
    ...
    RectArea(vct<coords> &c): n(c.size()), tag(4 * MAXN), scr(4 * MAXN), events(2 * c.size()) {
        rep(i, n) {
            coords curr = c[i];
...
signed main() {
    ...
    int n; cin >> n;
    vct<coords> coords(n);
    rep(i, n) cin >> coords[i].x1 >> coords[i].y1 >> coords[i].x2 >> coords[i].y2;
