...
struct DiffArr {
    ...
    DiffArr(vi &v): n(v.size()), arr(v.begin(), v.end()), diffs(v.size()) {}
...
public:
    bool isZeroArray(vector<int>& nums, vector<vector<int>>& queries) {
        int n = nums.size();
        DiffArr diffArr(nums);

        for (auto &qu: queries) diffArr.add(qu[0], qu[1], -1);
        vi res = diffArr.values();
        rep(i, n) if (res[i] > 0) return false;
        return true;
    }
};
