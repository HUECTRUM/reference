...
struct BinLift {
    ...
    BinLift(vvi &g, int LOG = 20, int root = 0): g(g), up(g.size(), vector<int>(LOG)), tin(g.size()), tout(g.size()), lg(LOG) {
        dfs(root);
    }
...
signed main() {
    ...
    function<void(int, int)> dfsD = [&](int x, int p) {
        if (p != -1) dist[x] = dist[p] + 1;
        repe(to, g[x]) if (to != p) dfsD(to, x);
    };
    dfsD(0, -1);
    ...
    while (q--) {
        int a, b; cin >> a >> b; --a, --b;
        int lc = bl.lca(a, b);
        int ans = dist[a] + dist[b] - 2 * dist[lc];
        cout << ans << "\n";
    }
}
