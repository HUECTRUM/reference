...
struct network_simplex {
    ...
    int add_edge(int u, int v, Flow upper, Cost cost) {
        Flow lower = 0;
        return add(u, v, lower, upper, cost);
    }
    ...
    void set_source(int u) { set_supply(u, INF); }
    void set_sink(int u) { add_demand(u, INF); }
    ...
    struct int_lists {
        ...
        int rre(int l) const { return l + N; }
        int head(int l) const { return next[rre(l)]; }
        int tail(int l) const { return prev[rre(l)]; }

        void push_front(int l, int n) { meet(rre(l), n, head(l)); }
        void push_back(int l, int n) { meet(tail(l), n, rre(l)); }
    ...
    void run() {
        ...
        block_size = max((int)(ceil(sqrt(E + V))), min((int)5, V + 1));
        next_arc = 0;
    ...
    void pivot(int in_arc) {
        ...
        for (i = 0, S = 1; i < S; i++) {
            int u = bfs[i];
            node[u].pi += pi_delta;
            for (int v = children.head(u); v != children.rre(u); v = children.next[v]) {
                bfs[S++] = v;
            }
        }
    }
...
signed main() {
    ...
    for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j) cin >> c, v[i][j] = c - '0';

    int totAll = 0, totLast = 0;
    for (int i = 0; i < n - 1; ++i) for (int j = 0; j < m; ++j) totAll += v[i][j];
    for (int j = 0; j < m; ++j) totLast += v[n - 1][j];
    if (totAll < totLast) {
        cout << "-1";
        return 0;
    }

    network_simplex<int, int> gr(n * m + 3);

    for (int i = 0; i < n - 1; ++i) {
        for (int j = 0; j < m; ++j) {
            int curr = i * m + 1 + j, fin = (n - 1) * m + 1 + j;
            if (v[i][j]) gr.add_edge(0, curr,  1, 0);
            if (v[n - 1][j]) gr.add_edge(curr, fin, 1, 0);

            if (j) gr.add_edge(curr, curr - 1,  INF, 1);
            if (j != m - 1) gr.add_edge(curr, curr + 1,  INF, 1);
        }
    }

    for (int j = 0; j < m; ++j) if (v[n - 1][j]) {
            int fin = (n - 1) * m + 1 + j;
            gr.add_edge(fin, n * m + 1,  INF, 0);
            gr.add_edge(fin, n * m + 2,  1, 0);
        }
    gr.add_edge(n * m + 1, n * m + 2, totAll - totLast, 0);
    gr.set_source(0);
    gr.set_sink(n * m + 2);

    int flow = gr.mincost_flow();
    if (flow != totAll) cout << "-1";
    else {
        cout << gr.get_circulation_cost();
    }
}