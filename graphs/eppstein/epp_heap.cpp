...
struct LeftistHeap {
    using self_t = LeftistHeap;
    int rank;
    int key;
    int value;
    self_t *left, *right;
    constexpr LeftistHeap(int rank_, int key_, int value_, self_t* left_,
                          self_t* right_)
            : rank{rank_}, key{key_}, value{value_}, left{left_}, right{right_} {}
    inline static std::deque<LeftistHeap> alloc;
    static self_t* insert(self_t* const a, const int k, const int v) {
        if (not a or k <= a->key) {
...
vector<int> epp(int n, const vector<vector<edge>> &g, int src, int dst, int k) {
    ...
    vector<LeftistHeap*> h(n, nullptr); queue<int> q({dst});
    while (q.size()) {
        ...
        for (auto [v, w] : g[u]) {
            ...
            else h[u] = LeftistHeap::insert(h[u], diff, v);
        }
    ...
    min_heap<pair<int, LeftistHeap*>> q1; q1.emplace(d[src] + h[src]->key, h[src]);
    while (not q1.empty() and (int) ans.size() < k) {
        auto [cd, ch] = q1.top(); q1.pop(); ans.push_back(cd);
        if (h[ch->value]) q1.emplace(cd + h[ch->value]->key, h[ch->value]);
...
signed main() {
    ...
    auto getIdx = [&](int i, int j, int r) {
        return i * 2 * (m + 1) + j * 2 + r;
    };
    vector<vector<edge>> g((n + 1) * (m + 1) * 2 + 1);
    rep(i, n + 1) rep(j, m + 1) rep(rr, 2) {
                int curIdx = getIdx(i, j, rr);
                if (rr && j < m) {
                    //move right
                    int nxtIdx = getIdx(i, j + 1, rr);
                    g[curIdx].emplace_back(nxtIdx, 0);
                }
                //move down
                if (i < n) {
                    int dwnIdx = getIdx(i + 1, j, 0);
                    g[curIdx].emplace_back(dwnIdx, 0);
                }

                if (i < n && j < m && s1[i] == s2[j]) {
                    int cst = v1[i] * v2[j];
                    int nxtIdx = getIdx(i + 1, j + 1, 1);
                    g[curIdx].emplace_back(nxtIdx, -cst);
                }
            }
    int sink = g.size() - 1;
    g[getIdx(n, m, 0)].emplace_back(sink, 0);
    g[getIdx(n, m, 1)].emplace_back(sink, 0);

    int st = getIdx(0, 0, 1);
    vi ans = epp(g.size(), g, st, sink, k);
