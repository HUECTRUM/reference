...
struct DpthOps: s2lContainer<int> {
    vector<int> vals;
    ...
    int getResult(int dpth) { return vals[dpth] - 1; }

    void resize(int x) { vals = vector<int>(x); }
};
...
template<typename R> struct Sack {
    ...
    Sack(int q, const vector<vector<int>> &g, s2lContainer<R> &ds, const vector<vector<pair<int, int>>> &queries):
    ds(ds), n(g.size()), g(g), sz(n), st(n), ft(n), ver(n), queries(queries), ans(q) {}
    ...
    void dfs(int v, int p, bool keep) {
        ...
        ds.add(v);
        for (auto &[dpth, idx]: queries[v]) {
            ans[idx] = ds.getResult(dpth);
        }
...
signed main() {
    ...
    vvpii queries(n);
    bl = BinLift(g);
    for (auto &t: roots) bl.dfs(t);

    int q; cin >> q;
    rep(i, q) {
        int v, diff; cin >> v >> diff; --v;

        int curd = bl.depth[v];
        if (diff > curd) continue;
        int anc = bl.kthAns(v, diff);
        queries[anc].push_back({curd, i});
    }

    DpthOps ops;
    Sack s(q, g, ops, queries);
    for (auto &t: roots) s.run(t);

    for (auto &ans: s.ans) cout << ans << " ";
}