...
struct Galki {
    ...
    Galki(int n, vvpii &gr): n(n), g(gr), color(n) {}
    ...
    int dfs(int x, int pe = -1) {
        ...
        repe2(to, edgeNum, g[x]) {
            if (color[to] == 2 || (!color[to] && dfs(to, edgeNum))) edgeNums.pb(edgeNum);
        }
        if (pe != -1) edgeNums.pb(pe);

        int i = 0;
        while (i + 1 < edgeNums.size()) res.pb({edgeNums[i], edgeNums[i + 1]}), i += 2;
...
signed main() {
    ...
    rep(i, m) if (ans[i].empty()) ans[i] = "x+";
    rep(i, m) cout << ans[i] << "\n";
}