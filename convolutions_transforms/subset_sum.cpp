...
struct BitConvolutions {
    void fwht(vi &arr, int sz, bool inverse = false) {
        ...
        if (inverse) rep(i, sz) arr[i] /= sz;
    }

    void fwht(vmint &arr, int sz, Mint szInv = 0, bool inverse = false) {
        for (int len = 1; 2 * len <= sz; len <<= 1) {
            for (int i = 0; i < sz; i += 2 * len) {
                rep(j, len) {
                    Mint u = arr[i + j], v = arr[i + j + len];
                    arr[i + j] = u + v, arr[i + j + len] = u - v;
                }
            }
        }

        if (inverse) rep(i, sz) arr[i] *= szInv;
    }

    vmint orconv(vmint &a, vmint &b, int sz) {
        fwht(a, sz); fwht(b, sz);
        vmint c(sz); rep(i, sz) c[i] = a[i] * b[i];

        Mint szInv = (Mint)1 / sz;
        fwht(c, sz, szInv, 1);
        return c;
    }
} bc;
...
struct AnnoyingTransforms {
    vi zeta(vi f, int n) {
        rep(i, n) rep(mask, (1 << n)) if ((mask & (1 << i))) f[mask] += f[mask ^ (1 << i)];
        return f;
    }

    vmint zeta(vmint f, int n) {
        rep(i, n) rep(mask, (1 << n)) if ((mask & (1 << i))) f[mask] += f[mask ^ (1 << i)];
        return f;
    }

    void zetaInPlace(vmint &f, int n) {
        rep(i, n) rep(mask, (1 << n)) if ((mask & (1 << i))) f[mask] += f[mask ^ (1 << i)];
    }

    vi supersetZeta(vi f, int n) {
        rep(i, n) rep(mask, (1 << n)) if (!(mask & (1 << i))) f[mask] += f[mask ^ (1 << i)];
        return f;
    }

    vi zeta2(vi f, int n) { //a[j] += a[i] when j&i=i
        rep(i, n) reprv(mask, (1 << n)) if (!(mask & (1 << i))) f[mask] -= f[mask ^ (1 << i)];
        return f;
    }

    template <int (*op)(int, int)> vi zeta(vi f, int n) {
        rep(i, n) rep(mask, (1 << n)) if ((mask & (1 << i))) f[mask] = op(f[mask], f[mask ^ (1 << i)]);
        return f;
    }

    vi mobius(vi f, int n) {
        rep(i, n) rep(mask, (1 << n)) if ((mask & (1 << i))) f[mask] -= f[mask ^ (1 << i)];
        return f;
    }

    vmint mobius(vmint f, int n) {
        rep(i, n) rep(mask, (1 << n)) if ((mask & (1 << i))) f[mask] -= f[mask ^ (1 << i)];
        return f;
    }

    void mobiusInPlace(vmint &f, int n) {
        rep(i, n) rep(mask, (1 << n)) if ((mask & (1 << i))) f[mask] -= f[mask ^ (1 << i)];
    }

    vmint supersetMobius(vmint f, int n) {
        rep(i, n) rep(mask, (1 << n)) if (!(mask & (1 << i))) f[mask] -= f[mask ^ (1 << i)];
        return f;
    }
} at;
...
struct SubsetSumConv {
    vmint ssc(vmint &a, vmint &b, int n, int sz) {
        vct<vmint> fhat(n + 1, vmint(sz)), ghat(n + 1, vmint(sz)), h(n + 1, vmint(sz));
        rep(i, sz) fhat[__builtin_popcount(i)][i] = a[i], ghat[__builtin_popcount(i)][i] = b[i];

        rep(i, n + 1) at.zetaInPlace(fhat[i], n), at.zetaInPlace(ghat[i], n);

        rep(mask, sz) rep(i, n + 1) rep(j, i + 1) h[i][mask] += fhat[j][mask] * ghat[i - j][mask];

        rep(i, n + 1) at.mobiusInPlace(h[i], n);

        vmint res(sz);
        rep(mask, sz) res[mask] = h[__builtin_popcount(mask)][mask];
        return res;
    }
} ssc;
