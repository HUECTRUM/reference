...
void toBin(vector<int> &v) {
    for (long long &x : v) if (x) x = 1;
}

vector<int> binPow(vector<int> a, int n) {
    vector<int> res = {1};
    while (n) {
        int resSz = res.size(), aSz = a.size();
        if (n & 1) {
            res = convolution(res, a);
            res.resize(resSz + aSz - 1);
            toBin(res);
        }
        a = convolution(a, a); a.resize(aSz * 2 - 1);
        toBin(a);
        n >>= 1;
    }
    return res;
...
signed main() {
    ...
    vector<int> ans = binPow(v, k);
    for (int i = 0; i < (int)ans.size(); ++i) if (ans[i]) cout << i << " ";
}