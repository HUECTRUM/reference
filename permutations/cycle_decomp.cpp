...
signed main() {
    ...
    while (tests--) {
        int n; cin >> n;
        vi perm(n), ord(n), ans(n);
        rep(i, n) cin >> perm[i], --perm[i];
        rep(i, n) cin >> ord[i], --ord[i];

        PermCycleDecomp cycd(perm);
        cycd.run();

        int cur = 0;
        si cycles;
        rep(i, n) {
            int curCyc = cycd.cycleIdx[ord[i]];
            if (!cycles.contains(curCyc)) {
                cycles.insert(curCyc);
                cur += cycd.cycleSize[curCyc];
            }
            ans[i] = cur;
        }
        rep(i, n) cout << ans[i] << " ";
