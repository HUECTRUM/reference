...
signed main() {
    ...
    while (tests--) {
        ...
        vct<vvmint> dp(n, vvmint(m, vmint(2))), sm(n, vvmint(m, vmint(2)));
//        Mint dp[n][m][2]; Mint sm[n][m][2];
//        memset(dp, 0, sizeof(dp));
//        memset(sm, 0, sizeof(sm));

        auto getSum = [&](int lvl, int k, int l, int r) {
            l = max(l, 0ll);
            r = min(r, m - 1);
            return sm[lvl][r][k] - (l ? sm[lvl][l - 1][k] : (Mint)0);
        };
        auto buildPrf = [&](int lvl, int k) {
            sm[lvl][0][k] = dp[lvl][0][k];
            reps(i, 1, m) sm[lvl][i][k] = dp[lvl][i][k] + sm[lvl][i - 1][k];
        };

        rep(j, m) if (v[n - 1][j]) dp[n - 1][j][0] = 1;
        buildPrf(n - 1, 0);
        ...
        reprv(i, n) rep(k, 2) {
                ...
                rep(j, m) {
                    ...
                    if (k) val += getSum(lvl, 0, l, r) - dp[i][j][0];
                    else val += getSum(lvl, 1, l, r) + getSum(lvl, 0, l, r);
