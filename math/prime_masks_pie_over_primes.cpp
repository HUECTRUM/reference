...
vvi primeDivs(MAXN, vi(20));
vi primeDivsSz(MAXN), isPrime(MAXN, 1);

void sieve() {
    reps(i, 2, MAXN) if (isPrime[i]) {
            for (int j = i; j < MAXN; j += i) isPrime[j] = 0, primeDivs[j][primeDivsSz[j]++] = i;
        }
}

vi cnt(MAXN), idxs(MAXN);

int totCnt = 0, totSum = 0, ans = 0;

void process(vi &v, int idx) {
    int x = v[idx];
    int curCnt = totCnt, curSum = totSum;

    reps(mask, 1, (1ll << primeDivsSz[x])) {
        int curD = 1;
        rep(j, primeDivsSz[x]) if (mask & (1ll << j)) curD *= primeDivs[x][j];

        int bits = __builtin_popcount(mask);
        curCnt += cnt[curD] * (bits % 2 ? -1 : 1);
        curSum += idxs[curD] * (bits % 2 ? -1 : 1);
    }
    ans += curCnt * idx - curSum - curCnt;
}

void add(vi &v, int idx) {
    int x = v[idx];
    totCnt += 1, totSum += idx;
